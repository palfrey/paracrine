...
from .fs import make_directory, run_command, run_with_marker, set_file_contents_from_template
...
def certbot_for_host(hostname: str, email: str) -> None:
    ...
    if use_this_host("certbot"):
        ...
        if not live_path.exists():
            ...
            setup_venv(venv)
            set_file_contents_from_template("/opt/certbot/requirements.txt", "certbot_requirements.txt")
            run_with_marker("/opt/certbot/deps_installed", f"{pip} install -r /opt/certbot/requirements.txt")

            run_command(
                f"{certbot_bin} certonly --config-dir={certbot.joinpath('config')} --work-dir={certbot.joinpath('workdir')} --logs-dir={certbot.joinpath('logs')} -m {email} --agree-tos --non-interactive --no-eff-email --domains {hostname} --dns-route53"
            )
        else:
            run_with_marker("/opt/certbot/renew_marker",
                f"{certbot_bin} renew --config-dir={certbot.joinpath('config')} --work-dir={certbot.joinpath('workdir')} --logs-dir={certbot.joinpath('logs')} --dns-route53", max_age=timedelta(days=1)
            )
        ...
        return {
            ...
            "ssl-options": venv.joinpath(
                "lib/python3.9/site-packages/certbot_nginx/_internal/tls_configs/options-ssl-nginx.conf"
            )
            .open()
            .read(),
        }
