...
from paracrine.fs import build_with_command, download, download_and_unpack, link, make_directory, run_command, set_file_contents, set_file_contents_from_template
...
def do():
    ...
    res = download_and_unpack('https://git.pleroma.social/pleroma/pleroma/-/jobs/220705/artifacts/download?file_type=archive', "8b4e2ab17362c7b0ed3ca685e19d578ad842ac00cde2db7d8c54dfd5a4e05891", "pleroma-2.4.4.zip", "/opt/pleroma-2.4.4")
    ...
    config_changes = set_file_contents_from_template("/etc/pleroma/config.exs", "config.exs.j2", PLEROMA_HOST=LOCAL["PLEROMA_HOST"])
    db_changes = set_file_contents_from_template("/opt/pleroma/setup_db.psql", "setup_db.psql")
    ...
    build_with_command("/etc/apt/trusted.gpg.d/postgresql.gpg", "cat /etc/apt/trusted.gpg.d/postgresql.gpg.asc | gpg --dearmor > /etc/apt/trusted.gpg.d/postgresql.gpg", ["/etc/apt/trusted.gpg.d/postgresql.gpg.asc"])
    debian_repo("postgresql_org_repository", "deb http://apt.postgresql.org/pub/repos/apt bullseye-pgdg main")
    ...
    if release_changed:
        run_command("su pleroma -s $SHELL -lc \"./bin/pleroma_ctl migrate\"", directory="/opt/pleroma")

    link_change = link("/etc/systemd/system/pleroma.service", "/opt/pleroma/installation/pleroma.service")
    if link_change:
        systemctl_daemon_reload()
    systemd_set("pleroma", enabled=True, running=True, restart=release_changed or config_changes)
    ...
    nginx_changes = set_file_contents_from_template("/etc/nginx/sites-available/pleroma.conf", "pleroma.nginx.j2", PLEROMA_HOST=LOCAL["PLEROMA_HOST"])
    nginx_changes = link("/etc/nginx/sites-enabled/pleroma.conf", "/etc/nginx/sites-available/pleroma.conf") or nginx_changes
